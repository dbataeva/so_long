...
{
	ft_free_bonus(0, image);
	if (image->move_left)
		free(image->move_left);
	if (image->enemy)
		free(image->enemy);
	if (out == 0)
		exit (0);
...
int	check_textures(int fd)
{
	...
	if (fd < 0)
		return (3);
	close(fd);
	fd = open("textures/enemy64.xpm", O_RDONLY);
	if (fd < 0)
		return (3);
	close(fd);
	return (check_textures_continue(fd));
}
...
void	xpm_file_to_image(t_data *image, int width, int height)
{
	...
	if (!image->player_image || (width != 64) || (height != 64))
		if_exit(image, 2);
	image->move_right = image->player_image;
	image->player_with_exit_right = mlx_xpm_file_to_image(image->mlx,
			"textures/dolphin_and_exit64.xpm", &width, &height);
	xpm_file_to_image_continue(image, width, height);
	return ;
}
...
void	new_window(t_data *image)
{
	...
	image->move_right = NULL;
	image->move_left = NULL;
	...
	draw(image, 0, 0, 0);
	mlx_hook(image->mlx_win, 2, 1L << 0, key_press_bonus, image);
